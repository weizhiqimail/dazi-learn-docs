---
id: 01-Gateway-Endpoint-Peering-PrivateLink题-等
sidebar_position: 1
---

# C4-11-Gateway-Endpoint-Peering-PrivateLink题-Gateway-Endpoint-Peering-PrivateLink题与Gateway-Endpoint-Peering-PrivateLink题与Gateway-Endpoint-Peering-PrivateLink题

> 本篇是《C4-11-Gateway-Endpoint-Peering-PrivateLink题》的第1个分篇，主要包含：Gateway-Endpoint-Peering-PrivateLink题、Gateway-Endpoint-Peering-PrivateLink题、Gateway-Endpoint-Peering-PrivateLink题。
...
题库记录的正确选项定位为：VPC Peering：两个 VPC 之间的点到点私网连接。

Amazon VPC：AWS 中用户定义的逻辑隔离虚拟网络。

AWS Transit Gateway：连接多个 VPC 与本地网络的中心 Hub/云路由器。
...
题库记录的正确选项定位为：VPC Endpoint：让 VPC 私网访问支持的 AWS 服务，而无需经过公共 Internet。
...
题库记录的正确选项定位为：AWS PrivateLink：通过私有 IP/接口 Endpoint 私下发布和访问服务。
...
- 本篇梳理了Gateway-Endpoint-Peering-PrivateLink题相关的核心知识、适用场景与判断要点。
- 本篇梳理了Gateway-Endpoint-Peering-PrivateLink题相关的核心知识、适用场景与判断要点。
- 本篇梳理了Gateway-Endpoint-Peering-PrivateLink题相关的核心知识、适用场景与判断要点。
