...
# C3-16-AWS-高频服务选择决策树-使用方法与核心决策树与五步推理法与先按“对象”而不是服务名分类与Gateway-家族单独决策与Managed-Serverless-不能机械背与反关键词背题提醒与做完一道题后的复盘

> 本篇是《C3-16-AWS-高频服务选择决策树》的第1个分篇，主要包含：使用方法、核心决策树、五步推理法、先按“对象”而不是服务名分类、Gateway-家族单独决策、Managed-Serverless-不能机械背、反关键词背题提醒、做完一道题后的复盘。
...
部署前估算 → Pricing Calculator。

运行中监控 → CloudWatch。

事后成本分析 → Cost Explorer。
...
题目出现“无需管理服务器”时，还要继续问：它运行的是函数、容器、数据库、分析查询，还是 ETL。

Lambda、Fargate、DynamoDB、Athena、Glue 都可能具有较高托管/Serverless 特征，但解决的问题完全不同。

---

## 7-反关键词背题提醒

同一个词可能出现在多个服务中，例如 `Gateway`、`Managed`、`Serverless`。

必须先看它连接什么、处理什么数据、解决哪层问题。
...
至少写下：题目对象、核心约束、正确选项定位、最强干扰项为什么不对。
...
- 本篇梳理了核心决策树相关的核心知识、适用场景与判断要点。
- 本篇梳理了五步推理法相关的核心知识、适用场景与判断要点。
- 本篇梳理了先按“对象”而不是服务名分类相关的核心知识、适用场景与判断要点。
- 本篇梳理了Gateway-家族单独决策相关的核心知识、适用场景与判断要点。
- 本篇梳理了Managed-Serverless-不能机械背相关的核心知识、适用场景与判断要点。
- 本篇梳理了反关键词背题提醒相关的核心知识、适用场景与判断要点。
- 本篇梳理了做完一道题后的复盘相关的核心知识、适用场景与判断要点。
