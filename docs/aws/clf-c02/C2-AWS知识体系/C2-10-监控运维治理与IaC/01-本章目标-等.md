...
# C2-10-监控、运维、治理与-IaC-本章目标与Amazon-CloudWatch与AWS-CloudTrail与AWS-Config与CloudWatch-CloudTrail-Config-考试最高频三分法与AWS-Systems-Manager与AWS-CloudFormation与Infrastructure-as-Code（IaC）与AWS-Trusted-Advisor与AWS-Compute-Optimizer与AWS-Organizations与AWS-Control-Tower与AWS-Service-Catalog与AWS-Resource-Access-Manager（RAM）与AWS-Health-Service-Health与GlobalShop-运维治理图与本章最后要形成的判断方式与与后续章节的关系
