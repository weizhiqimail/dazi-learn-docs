---
id: C2-08-01-Authentication-与-Authorization与AWS-IAM与Root-User
sidebar_position: 1
---

# C2-08-01-Authentication-与-Authorization与AWS-IAM与Root-User

与Identity-based-与-Resource-based-Policy与Explicit-Deny-与权限评估与Least-Privilege与MFA与Access-Key-与临时凭证与AWS-STS与Federation-与-IAM-Identity-Center与Amazon-Cognito与AWS-Organizations-与-SCP与IAM-与-Organizations-的位置关系与GlobalShop-身份体系与高频对比与本章最后要形成的判断方式与与后续章节的关系

> 本篇是《C2-08-IAM-身份与访问控制》的第1个分篇，主要包含：本章目标、Authentication-与-Authorization、AWS-IAM、Root-User、IAM-User、Group、Role、Policy-权限到底写在哪里？

Identity-based-与-Resource-based-Policy、Explicit-Deny-与权限评估、Least-Privilege、MFA、Access-Key-与临时凭证、AWS-STS、Federation-与-IAM-Identity-Center、Amazon-Cognito、AWS-Organizations-与-SCP、IAM-与-Organizations-的位置关系、GlobalShop-身份体系、高频对比、本章最后要形成的判断方式、与后续章节的关系。

## 1-本章目标

安全的第一道问题不是“有没有防火墙”，而是 你是谁？
...
**正式名称：** AWS Identity and Access Management，中文为： AWS 身份与访问管理。

### 3.1-为什么需要它

企业不能让所有员工和应用共享 root 密码，也不能把所有人都设置成管理员。

需要统一管理 AWS 身份、权限和策略。

### 3.2-它是什么

IAM 是 AWS 的身份与访问控制核心服务。

它管理 IAM User、Group、Role、Policy 等对象，并执行权限授权。

### 3.3-GlobalShop-场景

GlobalShop 的开发、运维、财务、安全团队拥有不同权限。

EC2/Lambda 也通过 Role 获取访问 S3、DynamoDB 等服务的权限。
...
表示某个长期身份，可能对应员工或技术用户。

可以有控制台凭证或 Access Key。
...
> SCP 不是给 IAM User “增加权限”的普通 IAM Policy。

它主要限制成员账号中身份可获得的最大权限范围。
...
- 本篇梳理了AWS-IAM相关的核心知识、适用场景与判断要点。
- 本篇梳理了Root-User相关的核心知识、适用场景与判断要点。
- 本篇梳理了IAM-User、Group、Role相关的核心知识、适用场景与判断要点。
- 本篇梳理了Policy-权限到底写在哪里？

相关的核心知识、适用场景与判断要点。
- 本篇梳理了Identity-based-与-Resource-based-Policy相关的核心知识、适用场景与判断要点。
- 本篇梳理了Explicit-Deny-与权限评估相关的核心知识、适用场景与判断要点。
- 本篇梳理了Least-Privilege相关的核心知识、适用场景与判断要点。
- 本篇梳理了MFA相关的核心知识、适用场景与判断要点。
- 本篇梳理了Access-Key-与临时凭证相关的核心知识、适用场景与判断要点。
- 本篇梳理了AWS-STS相关的核心知识、适用场景与判断要点。
- 本篇梳理了Federation-与-IAM-Identity-Center相关的核心知识、适用场景与判断要点。
- 本篇梳理了Amazon-Cognito相关的核心知识、适用场景与判断要点。
- 本篇梳理了AWS-Organizations-与-SCP相关的核心知识、适用场景与判断要点。
- 本篇梳理了IAM-与-Organizations-的位置关系相关的核心知识、适用场景与判断要点。
- 本篇梳理了GlobalShop-身份体系相关的核心知识、适用场景与判断要点。
- 本篇梳理了高频对比相关的核心知识、适用场景与判断要点。
- 本篇梳理了本章最后要形成的判断方式相关的核心知识、适用场景与判断要点。
- 本篇梳理了与后续章节的关系相关的核心知识、适用场景与判断要点。

[返回本章总述](./C2-08-00-章节总述.md)｜[查看本章概述](./C2-08-99-章节概述.md)
