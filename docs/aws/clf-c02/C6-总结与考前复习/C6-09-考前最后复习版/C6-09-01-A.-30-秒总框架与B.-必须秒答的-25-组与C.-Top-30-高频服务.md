---
id: C6-09-01-A.-30-秒总框架与B.-必须秒答的-25-组与C.-Top-30-高频服务
sidebar_position: 1
---

# C6-09-01-A.-30-秒总框架与B.-必须秒答的-25-组与C.-Top-30-高频服务
...
2. **Amazon S3** — 高耐久、可扩展的对象存储，以 Bucket/Object 组织数据。

（题库曝光 83）

3. **AWS IAM** — AWS 身份、认证授权与权限策略核心服务。

（题库曝光 76）

4. **On-Demand Instances** — 无长期承诺、按使用付费的计算购买方式。

（题库曝光 46）

5. **Reserved Instances** — 通过 1/3 年承诺获得折扣的 EC2/RDS 等预留定价概念，具体机制需按服务区分。

（题库曝光 45）

6. **Amazon RDS** — 托管关系数据库服务，负责大量数据库基础设施运维。

（题库曝光 42）

7. **AWS Trusted Advisor** — 根据 AWS 最佳实践提供成本、安全、性能、可靠性等检查建议。

（题库曝光 41）

8. **Shared Responsibility Model** — AWS 与客户之间划分云安全和运维责任的模型。

（题库曝光 41）

9. **Spot Instances** — 利用 AWS 闲置计算容量的低价实例，但可能被中断。

（题库曝光 41）

10. **Amazon VPC** — AWS 中用户定义的逻辑隔离虚拟网络。

（题库曝光 37）

11. **Amazon DynamoDB** — Serverless NoSQL Key-Value/Document 数据库，适合高规模、低延迟访问。

（题库曝光 36）

12. **Amazon GuardDuty** — 基于多种日志与威胁情报进行持续威胁检测。

（题库曝光 35）

13. **AWS CAF** — 帮助组织规划和推进云采用与业务/组织转型，包含六个 Perspectives。

（题库曝光 33）

14. **AWS WAF** — Web 应用防火墙，过滤 HTTP(S) Web 请求。

（题库曝光 33）

15. **AWS Support** — AWS 技术支持与客户服务计划体系。

项目资料明确提示当前计划体系有更新。

（题库曝光 32）

16. **AWS Lambda** — 事件驱动的 Serverless 函数运行环境，无需管理服务器。

（题库曝光 31）

17. **Amazon Inspector** — 持续漏洞管理与工作负载漏洞扫描。

（题库曝光 30）

18. **Security Group** — 资源/ENI 级有状态虚拟防火墙。

（题库曝光 30）

19. **Amazon CloudWatch** — Metrics/Logs/Alarms 等运行时监控与可观测性服务。

（题库曝光 29）

20. **AWS Direct Connect** — 客户网络到 AWS 的专用网络连接，适合稳定、长期、可预测链路需求。

（题库曝光 28）

21. **AWS Well-Architected Framework** — 以六大 Pillars 评估和设计安全、可靠、高效、成本优化、可持续的 Workload。

（题库曝光 27）

22. **AWS CloudTrail** — 记录 AWS API/账号活动，回答谁在什么时候做了什么操作。

（题库曝光 27）

23. **AWS Shield** — AWS DDoS 防护服务。

（题库曝光 26）

24. **Amazon CloudFront** — AWS CDN，利用 Edge Location 缓存和分发内容。

（题库曝光 26）

25. **AWS Config** — 记录资源配置与变化，并可按规则检查配置合规性。

（题库曝光 25）

26. **AWS CloudFormation** — AWS Infrastructure as Code，用模板声明和部署资源。

（题库曝光 25）

27. **Amazon Aurora** — AWS 云原生关系数据库，兼容 MySQL/PostgreSQL 生态，强调高性能与高可用。

（题库曝光 25）

28. **AWS Organizations** — 集中组织和治理多个 AWS Account，并支持 Consolidated Billing、SCP 等。

（题库曝光 24）

29. **AWS Cost Explorer** — 分析历史/当前 AWS 成本和用量趋势。

（题库曝光 23）

30. **Amazon Connect** — 云联络中心服务。
...
[返回本章总述](./C6-09-00-章节总述.md)｜[查看本章概述](./C6-09-99-章节概述.md)
